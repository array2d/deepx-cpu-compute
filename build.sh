...
BUILD_DIR="/tmp/deepx/cpu-compute/${PLAT}"
BIN_NAME="deepx-cpu-compute-${PLAT}"
...
# 重命名产物为平台特定名
if [ -f "$BUILD_DIR/deepx-cpu-compute" ]; then
    mv "$BUILD_DIR/deepx-cpu-compute" "$BUILD_DIR/$BIN_NAME"
fi
